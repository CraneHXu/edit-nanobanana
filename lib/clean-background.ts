...
interface AnalyzeRegionComplexityWorkerSuccess {
...
type CleanBackgroundWorkerResponse =
  | CleanBackgroundWorkerSuccess
  | AnalyzeRegionComplexityWorkerSuccess
  | CleanBackgroundWorkerFailure;
  ...
  return new Promise((resolve, reject) => {
    ...
    worker.onmessage = async (event: MessageEvent<CleanBackgroundWorkerResponse>) => {
      ...
      if (payload.type !== 'success') {
        cleanup();
        reject(new Error(`Unexpected clean background worker response: ${payload.type}`));
        return;
      }
...
export async function estimateRegionComplexity(
  ...
  return new Promise((resolve, reject) => {
    ...
    worker.onmessage = (event: MessageEvent<CleanBackgroundWorkerResponse>) => {
      const payload = event.data;
      if (payload.type === 'error') {
        cleanup();
        reject(new Error(payload.message));
      ...
      if (payload.type !== 'complexity-success') {
        cleanup();
        reject(new Error(`Unexpected clean background worker response: ${payload.type}`));
        return;
      }
    ...
    const request: AnalyzeRegionComplexityWorkerRequest = {
      type: 'analyze-region-complexity',
      imageDataUrl,
      region: {
        sourceBounds: cloneBounds(region.sourceBounds),
